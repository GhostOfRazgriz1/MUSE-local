...
class ProactivityManager:
    ...
    async def generate_idle_nudge(self) -> dict | None:
        ...
        time_ctx = f"Current time: {now.strftime('%A, %H:%M UTC')}"
    ...
    async def compose_greeting(self) -> str:
        ...
        # Gather all context
        now = datetime.now(timezone.utc)
        time_str = now.strftime("%A, %B %d at %H:%M UTC")
        hour = now.hour
        ...
        agent_name = self._orch._parse_identity_field("name") or "Agent OS"
