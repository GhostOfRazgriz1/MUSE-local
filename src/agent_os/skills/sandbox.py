...
from agent_os.skills.manifest import SkillManifest
...
class SkillSandbox:
    ...
    async def _run_in_process(
        ...
        try:
            from agent_os_sdk.context import SkillContext
        except ImportError:
            sdk_ctx_path = Path(__file__).resolve().parent.parent.parent.parent / "sdk" / "agent_os_sdk" / "context.py"
    ...
    async def _run_in_subprocess(
        ...
        if process is None:
            ...
            process = await asyncio.create_subprocess_exec(
                sys.executable, "-m", "agent_os.skills._bootstrap",
    ...
    async def send(self, message) -> None:
        ...
        if msg_type == "memory_read":
            ...
            try:
                ...
                import httpx
                # Build the request URL using the pinned IP to prevent
                # DNS rebinding (second lookup returning a different IP).
                ...
                if _resolved_ip and _hostname:
                    ...
                    request_url = message.url.replace(
                        f"{_parsed.scheme}://{_parsed.netloc}",
                        f"{_parsed.scheme}://{_resolved_ip}:{_port}",
                        1,
                    )
                    extra_headers["Host"] = _parsed.netloc
