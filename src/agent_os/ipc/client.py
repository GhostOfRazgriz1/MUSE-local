...
transport that :mod:`agent_os.ipc.server` creates:

* **Windows** -- named pipe ``\\\\.\\pipe\\agent-os-{task_id}`` (with
...
from agent_os.ipc.protocol import (
    MessageType,
    deserialize_message,
    serialize_message,
)
...
class IPCClient:
    ...
    def __init__(self, task_id: str, ipc_dir: str = "") -> None:
        ...
        self._ipc_dir = Path(ipc_dir) if ipc_dir else Path.cwd() / ".agent-os" / "ipc"
    ...
    async def _connect_windows(self) -> None:
        ...
        pipe_name = rf"\\.\pipe\agent-os-{self.task_id}"
        try:
